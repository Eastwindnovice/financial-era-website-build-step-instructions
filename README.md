...
然后关于右上角的星球的一个图片素材，我通过 ChatGPT 将参考图中的那一个星球素材分离出来了。分离出来的星球红蓝的一个对比素材，如下图所示：
![alt text](image-12.png)
这样分离出来之后，将这一个素材图片给它编录在网站的那个源码里面，就完成了如下图所示的一个排版布局。当然这个排版布局还是需要通过不断的微调实现的。因为网站的排版布局并不是使用简单 HTML 源码，而是使用了另一套技术栈。所以说需要综合的考虑。
![alt text](image-13.png)
这样子网站的上半部分就实现完毕了。现在是只有形体，还没有实现对应的一个后端逻辑。一般用 AI 编码的话，个人开发者更倾向于是先把网站的 UI 设计做好，后端的功能实现 后续会非常容易的进行迭代。
